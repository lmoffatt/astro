...
void update_Jacobian(const Experiment* e,const ABC_Distribution_Model* f,
...
{
  auto J0=f->J(e,ParamCurr, P_expCurr,dx);
  if (J0.empty())
    {
      invalidJ=true;
    }
  else
    {
      ...
      invalidJ=false;
      nFeval+=nPar;
      w=f->weight(e,P_expCurr);
      priorG=f->PriorGradient(ParamCurr);
      epsilon=f->epsilon(e,P_expCurr);
    }
}
...
//  update_Jacobian(e,CL_,ParamCurr_,P_expCurr_,nPar_,J_
...
void update_Likelihoods(const Experiment* e,
    const ABC_Distribution_Model* f
                        ...
                        ,std::size_t& NFeval)
{
  ...
  P_exp_New=f->f(e,ParamNew,dts);
  if (P_exp_New.empty())
    logLikNew=std::numeric_limits<double>::quiet_NaN();
  else
    logLikNew=f->logLik(e,P_exp_New);
...
//  J_.clear();
//  update_Likelihoods(e,CL_,ParamCurr_,P_expCurr_,logLikCurr_,logPriorCurr_,logPostLikCurr_
//                     ,nFeval_);
//}
...
//  update_Likelihoods(e,CL_,ParamNew_,P_exp_New_,logLikNew_,logPriorNew_,logPostLogLikNew_,nFeval_);
//}
...
unsigned ABC_Freq_obs::numSamples(const Experiment *e) const
{
  return ntot_obs(e).size();
}

unsigned ABC_Freq_obs::numCells(const Experiment* e) const
{

  if (!ntot_obs(e).empty())
    return ntot_obs(e).size()*n_obs(e)[0].size();
  else return 0;
}

std::size_t ABC_Freq_obs::numDF(const Experiment * e) const
{
  std::size_t sum=0;
  for (std::size_t i =0; i<ntot_obs(e).size(); ++i)
    {
      if (ntot_obs(e)[i]>0)
        sum+=n_obs(e)[i].size()-1;
    }
  return sum;
}
...
(const Experiment* e,const std::vector<double> &p,
 std::tuple<double, std::size_t, double> &dt_n_dtmax,
 std::size_t dts_max,
 std::pair<std::vector<double>,std::vector<std::size_t>>& dts)const

{
  return f(e,getPrior().toParameters(p),dt_n_dtmax,dts_max,dts);

}

std::vector<std::vector<double> > ABC_Distribution_Model::f(const Experiment* e,
    const std::vector<double> &p) const
{
  std::pair<std::vector<double>, std::vector<std::size_t>> dts;
  return f(e,getPrior().toParameters(p),dts);
}
std::vector<std::vector<double> > ABC_Distribution_Model::f(const Experiment* e,const std::vector<double> &p, std::pair<std::vector<double>,std::vector<std::size_t>>& dts) const
{
  return f(e,getPrior().toParameters(p),dts);
}

std::vector<std::vector<double> >
ABC_Distribution_Model::J(const Experiment *e, const Parameters &p,
                          const std::vector<std::vector<double> > &f0
                          , double delta) const
{


  std::vector<std::vector<double>> out(getData().numCells(e)
  ...
  for (std::size_t i=0; i< p.size(); ++i)
    {
      ...
      auto f_delta=f(e,pr,dts);
      if (f_delta.empty())
        return {};
      for (unsigned ii=0; ii<getData().ntot_obs(e).size(); ++ii)
        {
          for (unsigned j=0; j<getData().n_obs(e)[ii].size(); ++j)
            {
              if (f0[ii][j]>0)
                {
...
double ABC_Distribution_Model::logLik(const Experiment* e,const Parameters &p) const
{
  std::pair<std::vector<double>, std::vector<std::size_t>> dts;
  return logLik(e,f(e,p,dts));
}

double ABC_Distribution_Model::logLik(const Experiment *e, const std::vector<double> &o) const
{
  std::pair<std::vector<double>, std::vector<std::size_t>> dts;

  return logLik(e,f(e,getPrior().toParameters(o),dts));
}



std::vector<std::vector<double> >
ABC_Multinomial_Model::logLikCells(const Experiment* e,const std::vector<std::vector<double> > &p) const
{
  std::vector<std::vector<double> > cL(p.size(),std::vector<double>(p[0].size()));
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
    {
      double n=getData().ntot_obs(e)[i];
      for (unsigned j=0; j<getData().n_obs(e)[i].size(); ++j)
        {
          double nj=getData().n_obs(e)[i][j];
          if (nj>0)
...
std::vector<double> ABC_Multinomial_Model::logLikSamples(const Experiment* e,const std::vector<std::vector<double> > &p) const
{
...
double ABC_Multinomial_Model::logLik(const Experiment* e,const std::vector<std::vector<double> > &p) const
{
  double sumL=0;
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
    {
...
std::vector<double> ABC_Multinomial_Model::epsilon(const Experiment *e, const std::vector<std::vector<double> > &p) const
{
  std::vector<double> out(getData().numCells(e));
  unsigned ic=0;
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
    {
      double n=getData().ntot_obs(e)[i];
...
const std::vector<double> ABC_Multinomial_Model::weight(const Experiment* e,const std::vector<std::vector<double> > &p) const
{
  std::vector<double> out(getData().numCells(e));
  unsigned ic=0;
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
...
std::vector<std::vector<double> >
ABC_MultiPoison_Model::logLikCells(const Experiment *e, const std::vector<std::vector<double> > &landa) const
{
...
std::vector<double> ABC_MultiPoison_Model::logLikSamples(const Experiment* e,const std::vector<std::vector<double> > &landa) const
{
...
double ABC_MultiPoison_Model::logLik(const Experiment* e, const std::vector<std::vector<double> > &landa) const
{
  double sumL=0;
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
    {
...
double ABC_MultiPoison_Model::logLik(const Experiment* e,const Parameters &p) const
{
  std::pair<std::vector<double>, std::vector<std::size_t>> dts;
  return logLik(e,f(e,p,dts));
}

std::vector<double> ABC_MultiPoison_Model::epsilon(const Experiment* e,const std::vector<std::vector<double> > &landa) const
{
  std::vector<double> out(getData().numCells(e));
  unsigned ic=0;
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
...
const std::vector<double> ABC_MultiPoison_Model::weight(const Experiment *e,const std::vector<std::vector<double> > &landa) const
{
  std::vector<double> out(getData().numCells(e));
  unsigned ic=0;
  for (unsigned i=0; i<getData().ntot_obs(e).size(); ++i)
